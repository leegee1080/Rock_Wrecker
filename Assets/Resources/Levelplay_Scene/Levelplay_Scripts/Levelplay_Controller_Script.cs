...
public class Levelplay_Controller_Script : MonoBehaviour
{
    ...
    private void Gen_Map_Coords(int seed)
    {
        ...
        for (int x = 0; x < map_x_size; x++)
        {
            ...
            for (int y = 0; y < map_y_size; y++)
            {
                ...
                // noise_array[x+y] = new_y_grid_data[y].noise_data;
    ...
    private void Gen_Map_Residents()
    {
        ...
        foreach(Grid_Data[] y in x_lead_map_coord_array)
        {
            ...
            {
                if(item.noise_data < Mathf.Lerp(max_map_noise,min_map_noise,void_percentage/100f))
                {
                    if(item.noise_data > Mathf.Lerp(max_map_noise,min_map_noise,wall_percentage/100f) ||  item.grid_pos.x == 0 || item.grid_pos.x == map_x_size || item.grid_pos.y == 0 || item.grid_pos.y == map_y_size)
                    {
                    ...
                    if(player_placed == false && item.noise_data > min_map_noise && item.noise_data < Mathf.Lerp(max_map_noise,min_map_noise,rock_percentage/100f))
                    {
                        current_player.Place_Resident(item.grid_pos);
                        player_placed = true;
                        continue;
                    }
